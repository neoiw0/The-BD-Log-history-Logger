...
//1)Above the battle log,in the black bar, you will see how many times your multi-healers healed.(Make it easier for oneself to follow the DDL(https://www.neopets.com/~yotoive)/EEL(https://www.neopets.com/~EELRules)/OPT(https://www.neopets.com/~Airistole) rules)(as long as its used in its heal zone, it is not able do deal with 2 multihealers(eg. wodf+istaff) used same time in both's heal zones right now)
...
if((!logHTML.includes("https://images.neopets.com/items"))&&(!logHTML.includes("https://images.neopets.com/bd2/abili"))&&(!logHTML.includes("pets.neopets.com/cp"))){
    console.log("Start to add Icons");
for (const [weapon, icon] of Object.entries(weaponIcons)) {//<b>Scroll of The Dark Star</b>abilityIcons
    var weaeponModified;
    if(weapon=="You"){weaeponModified=weapon}
    else{weaeponModified="<b>"+weapon+"</b>";}
    const regex = new RegExp(`(${weaeponModified})`, 'g');
    logHTML = logHTML.replace(regex, `$1 <img src="${icon}" alt="${"weapon0"} icon" style="width:25px;height:25px;">`);
    ///////////Ability


}
      for (const [weapon, icon] of Object.entries(abilityIcons)) {//<b>Scroll of The Dark Star</b>abilityIcons

    if(weapon=="You"){weaeponModified=weapon}
    else{weaeponModified=""+weapon+"</i>";}
    const regex = new RegExp(`(${weaeponModified})`, 'g');
    logHTML = logHTML.replace(regex, `$1 <img src="${icon}" alt="${"ability0"} icon" style="width:25px;height:25px;">`);
      }
